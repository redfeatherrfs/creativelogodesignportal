<div class="bd-b-one bd-c-black-stroke pb-20 mb-20 d-flex align-items-center flex-wrap justify-content-between g-10">
    <h2 class="fs-18 fw-600 lh-22 text-title-text">{{ __('Update Membership Benefits') }}</h2>
    <div class="mClose">
        <button type="button"
...
<form class="ajax reset" action="{{ route('admin.theme-settings.membership-benefits.store') }}" method="post"
      data-handler="commonResponseWithPageLoad" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="id" value="{{$membershipBenefitsData->id}}">
    <div class="row rg-20">
        <div class="col-6">
            <label class="zForm-label">{{ __('Title') }} <span
                    class="text-danger">*</span></label>
            <input type="text" class="form-control zForm-control" name="title" value="{{ $membershipBenefitsData->title }}"
        ...
        <div class="col-6">
            ...
            <select name="status" class="sf-select-without-search">
                <option {{ $membershipBenefitsData->status == STATUS_ACTIVE ? 'selected' : '' }} value="{{STATUS_ACTIVE}}">{{ __('Active') }}</option>
                <option {{ $membershipBenefitsData->status == STATUS_DEACTIVATE ? 'selected' : '' }} value="{{STATUS_DEACTIVATE}}">{{ __('Deactivate') }}</option>
            </select>
        </div>
        <div class="col-12">
            <label class="zForm-label">{{ __('Description') }} <span
                    class="text-danger">*</span></label>
            <textarea class="form-control zForm-control"  name="description" rows="4" cols="50">{!! $membershipBenefitsData->description !!}</textarea>
        </div>
        <div class="col-12">
            <div class="primary-form-group">
                <div class="primary-form-group-wrap zImage-upload-details mw-100">
                    ...
                    <div class="upload-img-box">
                        <img src="{{ getFileUrl( $membershipBenefitsData->icon ) }}" />
                        <input type="file" name="icon" id="icon" accept="image/*"
                               onchange="previewFile(this)" />
                    </div>
                    <span
                        class="fs-12 fw-400 lh-24 text-main-color pt-3">{{__("Recommended: 72 px / 72 px")}}</span>
                </div>
            </div>
        </div>
    </div>
