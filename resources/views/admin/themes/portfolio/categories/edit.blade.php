<div class="bd-b-one bd-c-black-stroke pb-20 mb-20 d-flex align-items-center flex-wrap justify-content-between g-10">
    <h2 class="fs-18 fw-600 lh-22 text-title-text">{{ __('Update Portfolio Category') }}</h2>
    <div class="mClose">
        <button type="button"
...
<form class="ajax reset" action="{{ route('admin.theme-settings.portfolios.categories.store') }}" method="post"
      data-handler="commonResponseWithPageLoad" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="id" value="{{$portfolioCategoryData->id}}">
    <div class="row rg-20">
        <div class="col-12">
            <label class="zForm-label">{{ __('Name') }} <span
                    class="text-danger">*</span></label>
            <input type="text" class="form-control zForm-control" name="name" value="{{ $portfolioCategoryData->name }}"
                   placeholder="{{ __('Name') }}">
        </div>
        <div class="col-12">
            ...
            <select name="status" class="sf-select-without-search">
                <option {{ $portfolioCategoryData->status == STATUS_ACTIVE ? 'selected' : '' }} value="{{STATUS_ACTIVE}}">{{ __('Active') }}</option>
                <option {{ $portfolioCategoryData->status == STATUS_DEACTIVATE ? 'selected' : '' }} value="{{STATUS_DEACTIVATE}}">{{ __('Deactivate') }}</option>
            </select>
        </div>
    </div>
