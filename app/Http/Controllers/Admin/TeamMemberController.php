...
class TeamMemberController extends Controller
{
   use ResponseTrait;
    ...
    public function index(Request $request)
    {
        ...
        $data['allTeamMemberCount'] = User::where('role', USER_ROLE_TEAM_MEMBER)->count();
        if ($request->ajax()) {
            $data = User::where('users.role', USER_ROLE_TEAM_MEMBER)
                ...
                ->addColumn('action', function ($data) {
                    return "<div class='dropdown dropdown-one'>
                            ...
                            <ul class='dropdown-menu dropdownItem-two'>
                                <li>
                                    <a class='d-flex align-items-center cg-8' href='" . route('admin.team-member.edit', encodeId($data->id)) . "'>
                                        <div class='d-flex'>
                                ...
                                <li>
                                    <a class='d-flex align-items-center cg-8 delete' type='button' data-url='" . route('admin.team-member.delete', encodeId($data->id)) . "'>
                                        <div class='d-flex'>
    ...
    public function edit($id)
    {
        ...
        $data['activeTeamMember'] = 'active';
        $data['designations'] = Designation::where('status', STATUS_ACTIVE)->get();
        $data['teamMember'] = User::where('role', USER_ROLE_TEAM_MEMBER)->find(decodeId($id));
        $data['roles'] = Role::where('status', STATUS_ACTIVE)->get();
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            "email" => 'bail|required|unique:users,email,'. decodeId($request->id).',id,deleted_at,NULL',
            'designation_id' => 'required',
            'role' => 'required',
            'password' => decodeId($request->id) ? 'nullable': 'required|min:6',
        ]);
        DB::beginTransaction();
        try {
            $id = $request->get('id', '');
