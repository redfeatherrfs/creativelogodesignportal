...
class SUXVY extends Command
{
     ...
     * @var string
     */
    protected $signature = 'addoncommand {--lqs=} {--v=}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            $dbBuildVersion = getCustomerAddonBuildVersion('ADDONCODE');
            $v = $this->option('v');

            $logger->log('Command addoncommand', '===========START==========');
            ...
            if ($dbBuildVersion < $v) {
                ...
                setCustomerAddonBuildVersion('ADDONCODE', $v);
                setCustomerAddonCurrentVersion('ADDONCODE');
                DB::commit();
                $logger->log('Command RUN', 'DONE');
                $logger->log('Command addoncommand', '===========END==========');
                return true;
            } else {
                DB::rollBack();
